      ...
      let maybeSecondRequest = request.clone(), routeToUserWorker = async ({
        ...
        return eyeballConfig.limitedAssetsOnly ? (analytics.setData({ userWorkerFreeTierLimiting: !0 }), new Response(renderLimitedResponse(maybeSecondRequest), {
          ...
          if (shouldBlockNonImageResponse) {
            let resp = await env.USER_WORKER.fetch(maybeSecondRequest);
            return !resp.headers.get("content-type")?.startsWith("image/") && resp.status !== 304 ? (analytics.setData({ abuseMitigationBlocked: !0 }), new Response("Blocked", { status: 403 })) : resp;
          }
